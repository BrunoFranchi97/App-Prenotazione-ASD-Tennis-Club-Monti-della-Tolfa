...
import { format, parseISO, startOfHour, addHours, isEqual, isBefore, isAfter } from 'date-fns';
...
const BookingGrid: React.FC<BookingGridProps> = ({ 
  ...
  const timeSlots = useMemo(() => {
    const slots = [];
    for (let i = 8; i < 22; i++) {
      slots.push(format(setHours(new Date(), i), 'HH:00'));
    }
    return slots;
  }, []);
  ...
  const getReservation = (courtId: number, slotTime: string) => {
    const slotStart = format(parseISO(`${format(date, 'yyyy-MM-dd')}T${slotTime}:00`), 'yyyy-MM-dd HH:mm');
    return reservations.find(r => {
      const resStart = format(parseISO(r.starts_at), 'yyyy-MM-dd HH:mm');
      return r.court_id === courtId && resStart === slotStart;
    });
  };
  ...
  return (
    <div className="relative border border-gray-100 rounded-[2rem] bg-white overflow-hidden shadow-sm">
      <div 
        ref={scrollRef}
        className="overflow-auto max-h-[60vh] sm:max-h-[70vh] no-scrollbar"
      >
        <table className="w-full border-collapse table-fixed">
          ...
          <tbody className="relative">
            {timeSlots.map((slot) => {
              const isPast = isBefore(parseISO(`${format(date, 'yyyy-MM-dd')}T${slot}:00`), startOfHour(now));
              const isToday = format(date, 'yyyy-MM-dd') === format(now, 'yyyy-MM-dd');
              const isCurrentTime = isToday && slot === currentHourString;
                    ...
                    return (
                      <td 
                        ...
                        className={cn(
                          ...
                          isPast && "bg-gray-50/50 grayscale opacity-40",
                      ...
                      >
                        {res ? (
                          <div className={cn(
                            "absolute inset-1 rounded-xl p-2 flex flex-col justify-center overflow-hidden shadow-sm",
            ...
            {isToday && (
              <div 
                className="absolute left-0 right-0 h-px bg-club-orange z-10 pointer-events-none flex items-center"
                style={{ 
                  top: `${((now.getHours() - 8) * 56) + (now.getMinutes() / 60 * 56)}px`
                }}
              >
                <div className="w-2 h-2 rounded-full bg-club-orange -ml-1 shadow-sm" />
              </div>
            )}
          </tbody>
        </table>
