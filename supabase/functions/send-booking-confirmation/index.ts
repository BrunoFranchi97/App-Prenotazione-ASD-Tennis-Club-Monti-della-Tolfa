...
serve(async (req) => {
  ...
  try {
    const { userEmail, userName, courtName, reservations } = await req.json();

    console.log("[send-booking-confirmation] Received request to send email:", { userEmail, userName, courtName, reservations });
    ...
    //   to: userEmail,
    //   subject: 'Conferma Prenotazione Campo da Tennis',
    //   html: `
    //     <h1>Ciao ${userName},</h1>
    ...
    console.log(`[send-booking-confirmation] Simulazione invio email a ${userEmail} per la prenotazione del campo ${courtName} il ${bookingDate} dalle ${bookingStartTime} alle ${bookingEndTime}`);
